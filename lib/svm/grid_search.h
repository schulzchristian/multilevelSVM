...
{
public:
        grid_search(float c_from, float c_to, float c_step,
                    float g_from, float g_to, float g_step);
        virtual ~grid_search();
        std::vector<std::pair<float,float>> get_sequence();

        static grid_search around(float c_center, float c_range, float c_step,
                                  float g_center, float g_range, float g_step);

        static std::vector<std::pair<float,float>> mlsvm_method(float c_from, float c_to, float g_from, float g_to,
